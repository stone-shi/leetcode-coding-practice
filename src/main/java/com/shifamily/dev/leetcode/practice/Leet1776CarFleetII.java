...
public class Leet1776CarFleetII extends BasicStudy {
    ...
    public List<CaseParameters> data1() {
        ...
        cases.add(CaseParameters.builder()
                .parameters(new Object[] { new int[][] { { 1, 4 }, { 4, 5 }, { 7, 1 }, { 13,
                        4 }, { 14, 3 }, { 15, 2 },
                        { 16, 5 }, { 19, 1 } } })
                .answer(new Double[] { 2.00000, 0.75000, -1.00000, 1.00000, 1.00000, 4.00000,
                        0.75000, -1.00 })
    ...
    public double[] getCollisionTimes2(int[][] cars) {
        ...
        for (int i = n - 1; i >= 0; i--) {
            ...
            while (!stack.isEmpty()) {
                ...
                if (s <= s1 || (p1 - p) * 1.0 / (s - s1) > res[prev] && res[prev] > 0)
                    stack.pop();
                else
                    break;
            }
            if (!stack.isEmpty()) {
                int prev = stack.peek();
                res[i] = (cars[prev][0] - p) * 1.0 / (s - cars[prev][1]);
            }
            stack.push(i);
        }
        return res;
